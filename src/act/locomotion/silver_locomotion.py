...
from silver_msgs.msg import Command
from silver_msgs.srv import GaitSelectionAnna, GaitSelectionAnnaResponse, GaitSelector, GaitSelectorResponse, GetDxlStatus, GetDxlStatusResponse, Reboot, RebootResponse, Sampling, SamplingResponse, SetManipulation, SetManipulationResponse, Manipulation, ManipulationResponse
...
if len(sys.argv) is 2:
	phantom = int(sys.argv[1])
else: #bad argument number
	phantom = 1 #default is phantom mode to prevent hardware damages
...
class LocomotionController:
    ...
    def __init__(self):
        ...
        N=6
        manipulator = 1
        self.robot = robot.Robot(N, 3, parentDir+'/locomotion/trajectories/limits.txt','folded', phantom, manipulator)

        #state variables
        self.gait_type = None
        self.gait_go = False
        self.dxl_status = [0]*18
        self.touch=False
        ...
        self.contacts = 1.5*np.ones(6)
        self.contact = False
        self.time_last_contact = time.time()
        self.time_last_detach = time.time()
        self.last_cmd = time.time()
        self.virtual_legs = None
        self.next_virtual_leg = None
        # rotate gait parameters
        self.rotate_gait_params = None
        self.Rdirection = None
        # push from ground params
        self.push_params = None
        # Threshold on piezoelectric signals
        self.t_push = 1
        self.t_pull = 1
        self.td_threshold = 4 #[V]  OL 6
        self.lo_threshold = 3 #[V] OL -1
        # maximum time to complete pushing or pulling action, set it to Infinity to eliminate the open loop base layer
        self.t_max_pull = 4 #maximum allowed duration of swimming phase
        self.t_max_push = 2 #maximum allowed duration of punting phase
        #minimum time to complete pushing or pulling action, set it non zero in case of piezo multipeaks
        self.t_min_pull = 1 # if 0 condition for td event is always true [immediately as touch-sensors read smt]
        self.t_min_push = 1 # if 0 condition for lo event is always true
        ####################

        # create services
        #                available gaits
        self.set_gait_proxy = rospy.Service('silver_locomotion/set_gait', GaitSelector, self.set_gait)
        self.set_crabGait_proxy = rospy.Service('silver_locomotion/set_crab', GaitSelectionAnna, self.set_crab)
        ...
        #                status monitoring
        self.get_dxl_status_proxy = rospy.Service('silver_locomotion/get_dxl_status', GetDxlStatus, self.get_dxl_status)
        self.reboot_proxy = rospy.Service('silver_locomotion/reboot', Reboot, self.reboot)
        #                manipulation modes 
        self.sampling_proxy = rospy.Service('silver_locomotion/sampling', Sampling, self.sample)
        self.set_manipulation_proxy = rospy.Service('silver_locomotion/set_manipulation', SetManipulation, self.set_manipulation)
        self.chiccohand_proxy = rospy.Service('silver_locomotion/activate_hand', SetBool, self.activate_hand)
        self.autosampling_proxy = rospy.Service('silver_locomotion/autonomous_sampling', Empty, self.auto_sample)
